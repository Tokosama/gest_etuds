...
@section('header', 'Ajouter un Nouvel Élément Constitutif (EC)')

@section('content')
    <div class="container mx-auto py-12 px-6">
        <h1 class="text-center text-4xl font-extrabold text-gray-800 mb-12">Ajouter un Nouvel Élément Constitutif (EC)</h1>
        @if ($errors->any())
        ...
        <form action="{{ route('ecs.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="code" id="code" class="form-input mt-1 block w-full @error('code') border-red-500 @enderror" value="{{ old('code') }}" required>
                @error('code')
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nom" id="nom" class="form-input mt-1 block w-full @error('nom') border-red-500 @enderror" value="{{ old('nom') }}" required>
                @error('nom')
                    <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                @enderror
            </div>
            <div class="mb-4">
                ...
                <input type="number" name="coefficient" id="coefficient" step="0.01" class="form-input mt-1 block w-full @error('coefficient') border-red-500 @enderror" value="{{ old('coefficient') }}" required>
                @error('coefficient')
                    <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                @enderror
            </div>
            <div class="mb-4">
                ...
                <select name="ue_id" id="ue_id" class="form-select mt-1 block w-full @error('ue_id') border-red-500 @enderror" required>
                    @foreach($ues as $ue)
                        <option value="{{ $ue->id }}" {{ old('ue_id') == $ue->id ? 'selected' : '' }}>{{ $ue->code }} - {{ $ue->nom }}</option>
                    @endforeach
                </select>
                @error('ue_id')
                    <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
